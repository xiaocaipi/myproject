...
public class debug {
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		String inputPath="/home/spring/input/";//由于是本地调试，所以用本地的目录
		String outputPath="home/spring/output/";
		
		Job job=new Job();
		job.setJarByClass(debug.class);//设置job运行的class
		job.setJobName("CdrStat");//设置job的name
		
		//返回一个jobconfiguration的对象
		//得到对象之后来设置运行的变量,设置为本地，就不会提交给hadoop集群
		job.getConfiguration().set("mapred.job.tracker", "local");
		//设置另外一个参数，是输入文件的方式，默认的是去hdfs获得输入，并且将输出的文件写到hdfs上面，将其改成local,到本地文件去获取，以及输出到本地
