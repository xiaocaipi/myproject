package com.stock.vo;

public class StockRealTimeData extends StockData {
	
	private String time;
	
	private Double buy1shou;
	
	private Double buy2shou;
	
	private Double buy3shou;
	
	private Double buy4shou;
	
	private Double buy5shou;
	
	private Double buy1price;
	
	private Double buy2price;
	
	private Double buy3price;
	
	private Double buy4price;
	
	private Double buy5price;
	
	private Double sell1shou;
	
	private Double sell2shou;
	
	private Double sell3shou;
	
	private Double sell4shou;
	
	private Double sell5shou;
	
	private Double sell1price;
	
	private Double sell2price;
	
	private Double sell3price;
	
	private Double sell4price;
	
	private Double sell5price;
	
	private Double dealSumShou;//成交了多少手
	
	private Double dealnowShou;//一次变动成交了多少手
	
	private Double refclose;//昨天的收盘价
	
	private Double dealnowmoney;//一次变动成交了多少钱

	public String getTime() {
		return time;
	}

	public void setTime(String time) {
		this.time = time;
	}

	public Double getBuy1shou() {
		return buy1shou;
	}

	public void setBuy1shou(Double buy1shou) {
		this.buy1shou = buy1shou;
	}

	public Double getBuy2shou() {
		return buy2shou;
	}

	public void setBuy2shou(Double buy2shou) {
		this.buy2shou = buy2shou;
	}

	public Double getBuy3shou() {
		return buy3shou;
	}

	public void setBuy3shou(Double buy3shou) {
		this.buy3shou = buy3shou;
	}

	public Double getBuy4shou() {
		return buy4shou;
	}

	public void setBuy4shou(Double buy4shou) {
		this.buy4shou = buy4shou;
	}

	public Double getBuy5shou() {
		return buy5shou;
	}

	public void setBuy5shou(Double buy5shou) {
		this.buy5shou = buy5shou;
	}

	public Double getBuy1price() {
		return buy1price;
	}

	public void setBuy1price(Double buy1price) {
		this.buy1price = buy1price;
	}

	public Double getBuy2price() {
		return buy2price;
	}

	public void setBuy2price(Double buy2price) {
		this.buy2price = buy2price;
	}

	public Double getBuy3price() {
		return buy3price;
	}

	public void setBuy3price(Double buy3price) {
		this.buy3price = buy3price;
	}

	public Double getBuy4price() {
		return buy4price;
	}

	public void setBuy4price(Double buy4price) {
		this.buy4price = buy4price;
	}

	public Double getBuy5price() {
		return buy5price;
	}

	public void setBuy5price(Double buy5price) {
		this.buy5price = buy5price;
	}

	public Double getSell1shou() {
		return sell1shou;
	}

	public void setSell1shou(Double sell1shou) {
		this.sell1shou = sell1shou;
	}

	public Double getSell2shou() {
		return sell2shou;
	}

	public void setSell2shou(Double sell2shou) {
		this.sell2shou = sell2shou;
	}

	public Double getSell3shou() {
		return sell3shou;
	}

	public void setSell3shou(Double sell3shou) {
		this.sell3shou = sell3shou;
	}

	public Double getSell4shou() {
		return sell4shou;
	}

	public void setSell4shou(Double sell4shou) {
		this.sell4shou = sell4shou;
	}

	public Double getSell5shou() {
		return sell5shou;
	}

	public void setSell5shou(Double sell5shou) {
		this.sell5shou = sell5shou;
	}

	public Double getSell1price() {
		return sell1price;
	}

	public void setSell1price(Double sell1price) {
		this.sell1price = sell1price;
	}

	public Double getSell2price() {
		return sell2price;
	}

	public void setSell2price(Double sell2price) {
		this.sell2price = sell2price;
	}

	public Double getSell3price() {
		return sell3price;
	}

	public void setSell3price(Double sell3price) {
		this.sell3price = sell3price;
	}

	public Double getSell4price() {
		return sell4price;
	}

	public void setSell4price(Double sell4price) {
		this.sell4price = sell4price;
	}

	public Double getSell5price() {
		return sell5price;
	}

	public void setSell5price(Double sell5price) {
		this.sell5price = sell5price;
	}

	public Double getDealSumShou() {
		return dealSumShou;
	}

	public void setDealSumShou(Double dealSumShou) {
		this.dealSumShou = dealSumShou;
	}

	public Double getDealnowShou() {
		return dealnowShou;
	}

	public void setDealnowShou(Double dealnowShou) {
		this.dealnowShou = dealnowShou;
	}

	public Double getRefclose() {
		return refclose;
	}

	public void setRefclose(Double refclose) {
		this.refclose = refclose;
	}

	public Double getDealnowmoney() {
		return dealnowmoney;
	}

	public void setDealnowmoney(Double dealnowmoney) {
		this.dealnowmoney = dealnowmoney;
	}
	
	
}
